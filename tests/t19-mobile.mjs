...
// เครื่องมือแอดมินชุดใหม่ (ค้นหาออเดอร์ / กล่องประวัติรายคน)
ok("ช่องค้นหาออเดอร์กว้างเต็มบรรทัด", has(/#order-search\{[^}]*max-width:none/));
ok("แถวค้นหาขึ้นบรรทัดใหม่ได้ ไม่ดันจอ", has(/\.search-row\{[^}]*flex-wrap:wrap/));
ok("การ์ดตัวเลขในกล่องประวัติสมาชิกเหลือ 2 คอลัมน์",
  has(/\.panel-wide \.tiles\{[^}]*grid-template-columns:repeat\(2/));
ok("กล่องประวัติสมาชิกลดขอบในจอแคบ", has(/\.panel-wide\{[^}]*padding:18px/));
// ช่องค้นหาต้องยืดหดได้ในจอกว้างด้วย ไม่งั้นดันแถวจนจอ 320px เลื่อนซ้ายขวา (กฎเหล็กข้อ 15)
ok("ช่องค้นหาออเดอร์ยืดหดได้ (min-width:0)", /#order-search\{[^}]*min-width:0/.test(flatCss));
ok("แถวค้นหามี min-width:0", /\.search-row\{[^}]*min-width:0/.test(flatCss));
