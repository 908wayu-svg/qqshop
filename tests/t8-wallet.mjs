...
section("พร้อมเพย์ — ตรวจมาตรฐาน EMV แบบละเอียด");
const { promptPayPayload: pp } = await import("./sandbox/promptpay.mjs");
// ค่ามาตรฐานสากลของ CRC-16/CCITT-FALSE: crc("123456789") = 29B1
const crcOf = s => s.slice(-4);
const bodyOf = s => s.slice(0, -4);
const p0 = pp("0918200409", 0, "QQSHOP");
const p1 = pp("0918200409", 250.5, "QQSHOP");
ok("ไม่ระบุยอด = ใช้ซ้ำได้ (010211)", p0.startsWith("00020101021129"), p0.slice(0, 20));
ok("ระบุยอด = ใช้ครั้งเดียว (010212)", p1.startsWith("00020101021229"), p1.slice(0, 20));
ok("รหัสพร้อมเพย์ถูกต้อง", p0.includes("0016A000000677010111"));
ok("เบอร์แปลงเป็นรูปแบบสากล", p0.includes("01130066918200409"));
ok("สกุลเงินบาท (764)", p0.includes("5303764"));
ok("ประเทศไทย", p0.includes("5802TH"));
ok("ยอดเงินทศนิยม 2 ตำแหน่ง", p1.includes("5406250.50"));
ok("ลงท้ายด้วยช่อง CRC", p1.slice(-8, -4) === "6304");
ok("ความยาวรวมสมเหตุสมผล", p0.length > 60 && p0.length < 200, p0.length + " ตัว");
ok("เบอร์ที่มีขีดคั่นก็ใช้ได้", pp("091-820-0409", 0, "QQSHOP") === p0);
ok("เบอร์รูปแบบ 66 นำหน้าก็ได้ผลเดียวกัน", pp("66918200409", 0, "QQSHOP") === p0);
ok("เบอร์รูปแบบ +66 ก็ได้ผลเดียวกัน", pp("+66 91-820-0409", 0, "QQSHOP") === p0);
ok("เบอร์รูปแบบ 0066 ก็ได้ผลเดียวกัน", pp("0066918200409", 0, "QQSHOP") === p0);
ok("เลขบัตรประชาชน 13 หลักใช้ช่อง 02", pp("1234567890123", 0, "QQSHOP")?.includes("02131234567890123") === true,
   String(pp("1234567890123", 0, "QQSHOP")));
ok("เบอร์สั้นเกินไป = ไม่สร้าง QR มั่ว", pp("081234", 0, "QQSHOP") === null);
ok("เบอร์ยาวเกินไป = ไม่สร้าง QR มั่ว", pp("08123456789012345", 0, "QQSHOP") === null);
ok("ไม่ได้ตั้งเบอร์เลย = ไม่สร้าง QR มั่ว", pp("", 0, "QQSHOP") === null && pp(null, 0, "QQSHOP") === null);
ok("ตัวหนังสือล้วน = ไม่สร้าง QR มั่ว", pp("ยังไม่ได้ตั้ง", 0, "QQSHOP") === null);

section("เลขตรวจสอบท้าย QR ต้องถูกจริง (ไม่ใช่แค่มีครบ 4 หลัก)");
// ทุกข้อข้างบนตรวจ "โครง" ของ payload — ถ้าสูตรเลขตรวจสอบ (CRC) ผิด
// เทสจะยังผ่านหมด แต่ **แอปธนาคารของลูกค้าจะปฏิเสธ QR ทุกใบ** แล้วไม่มีใครรู้ว่าทำไม
//
// จึงเขียนสูตรขึ้นมาใหม่ในเทสด้วยวิธีคนละแบบ (ตารางค่าล่วงหน้า แทนการเลื่อนบิตทีละตัว)
// แล้วเทียบกัน — ถ้าลอกสูตรเดียวกันมาวาง จะผิดเหมือนกันทั้งคู่โดยไม่มีใครจับได้
{
  // CRC-16/CCITT-FALSE แบบตาราง
  const TABLE = (() => {
  ...
  // ค่ามาตรฐานสากลที่ใช้ตรวจสูตร CRC-16/CCITT-FALSE
  ok("สูตรอ้างอิงในเทสเองถูกต้อง (123456789 -> 29B1)", crcRef("123456789") === "29B1", crcRef("123456789"));

  for (const [name, code] of [
    ["ไม่ระบุยอด", p0],
    ["ระบุยอด 250.50", p1],
    ["ยอดเต็มจำนวน", pp("0918200409", 100, "QQSHOP")],
    ["ชื่อร้านยาว", pp("0918200409", 1, "ร้านชื่อยาวมากกกกกกกกกกกกกกกกก")],
    ["เลขบัตรประชาชน", pp("1234567890123", 20, "QQSHOP")],
  ]) {
    const body = code.slice(0, -4);
    ok("เลขตรวจสอบถูกต้อง · " + name, code.slice(-4) === crcRef(body),
      "ได้ " + code.slice(-4) + " ควรเป็น " + crcRef(body));
  }

  // เปลี่ยนข้อมูลนิดเดียว เลขตรวจสอบต้องเปลี่ยนตาม (พิสูจน์ว่ามันคิดจากเนื้อจริงๆ)
  ok("ยอดเงินต่างกัน เลขตรวจสอบต้องต่างกัน",
    pp("0918200409", 100, "QQSHOP").slice(-4) !== pp("0918200409", 101, "QQSHOP").slice(-4));
}
